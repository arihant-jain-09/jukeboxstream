...
// ---------------------------Colors--------------------------
router.get('/song/colors/:hashkey', async (req, res) => {
  const hashkey = req.params.hashkey;
  const result = await client.hgetall(itemColorKey(hashkey));
  console.log(result);
  res.status(200).json(result);
});

router.post('/song/colors/:hashkey', async (req, res) => {
  const { userId } = req.body;
  const hashkey = req.params.hashkey;
  const result = await client.hgetall(userItemColorKey(userId, hashkey));
  console.log(result);
  res.status(200).json(result);
});
